...
const Profile = () => {
	...
	// setting up as local state, not global state
...
// 2. Axios global setup
// Downside: it sends bearer token along with any request, even to external APIs
...
// we won't keep the headers field, we will move that to interceptors
// current setup doesn't give us a way to handle 401 errors
// 400 is bad request.  401 is authentication error
// thus use interceptors approach - which is a type of middleware
...
// Hence we get dirname, fileURLToPath, and path from node modules
